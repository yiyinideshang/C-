...
C++ 中输入输出用 `>>` 和 `<<` 而不是别的符号，是**历史选择、视觉隐喻、运算符重载机制**三者共同作用的结果。下面从几个角度帮你理清这个设计决策。
...
```cpp
std::cout << "Hello";   // 数据从右侧“流入” cout（输出流）
std::cin  >> variable;  // 数据从 cin（输入流）“流出”到 variable
```

- `<<` 箭头指向左侧的**流对象**，表示**把数据塞进流里**。
- `>>` 箭头指向右侧的**变量**，表示**把数据从流里拉出来**。

这种直观的方向性让代码读起来几乎像自然语言描述数据流动。

### 2. 借用了位运算符，且优先级合适

`<<` 和 `>>` 在 C 语言里原本是**位移运算符**（左移/右移）。C++ 支持**运算符重载**，设计者 Bjarne Stroustrup 决定复用它们来做 I/O。

为什么选这两个而不是其他符号？

- **优先级刚好“够用”但不完美**：位移运算符的优先级比算术运算符低，但比比较运算符高。这让大部分链式调用能正常解析：
  ```cpp
  std::cout << "sum = " << a + b;    // 先算 a+b，再输出
  std::cout << (a < b);              // 比较需要加括号，优先级此处不够低
  ```
  如果选了优先级更高的运算符（比如 `+`），写 `cout + a + b` 会先算 `a+b` 但难以区分字符串拼接；如果选优先级更低的（比如 `=`），会让表达式冗长。虽然 `<<` 的优先级也造成了一些坑（比如 `cout << a & b` 需要括号），但总体上是当时可用的**最佳折中方案**。

- **没有更合适的单字符运算符可用**：C++ 从 C 继承的运算符集合里，像 `.` `,` `:` 等要么不能被重载，要么语义完全不合。新造符号（比如 `<-` 和 `->`）会破坏与 C 的词法兼容性。而 `<<` `>>` 恰好是 C 已有的合法双字符运算符，重载它们不会引入新词法。

### 3. 运算符重载带来的链式调用和可扩展性

通过重载 `operator<<` 和 `operator>>`，C++ 实现了**类型安全的链式语法**：

```cpp
std::cout << "Hello, " << name << ", you are " << age << " years old.\n";
```

- 每个 `<<` 返回流对象的引用，所以可以一直 `.<<` 下去。
- 你可以为自己定义的类重载 `operator<<`，让自定义类型也能像内置类型一样用 `cout` 输出——这是 C 语言 `printf` 用格式化字符串无法做到的（`printf` 需要显式指定 `%d` 等格式，且不能直接打印自定义结构）。

### 4. 为什么不直接用函数（像 C 的 `printf` / `scanf`）？

`printf` 和 `scanf` 依赖**可变参数**和**格式化字符串**，在 C++ 中有三大缺陷：

- **类型不安全**：编译器无法检查格式字符串与参数是否匹配，易出错。
- **不可扩展**：用户无法给自定义类型增加 `%` 转换说明符。
- **性能开销**：运行时解析格式字符串。

C++ 的流 I/O 用运算符重载，在**编译期**就确定了调用哪个重载版本，既安全又高效（且可内联优化）。

### 5. 一个有趣的背景：早期 C++ 曾用过 `<<` 和 `>>` 以外的设计

在 C++ 非常早期的版本（带类的 C）中，Stroustrup 曾尝试用**成员函数**风格做 I/O，比如：

```cpp
cout.print("Hello");
```

但这种风格**无法链式调用**（除非每个函数都返回 `*this`），且不够简洁。引入运算符重载后，利用 `<<` `>>` 的符号化表达让代码**读起来更像数据在流动**，这一设计很快被接受并成为 C++ 的标志性语法之一。
...
| ------------ | ----------------------------------- |
| **视觉提示** | 像箭头，表示数据进出流的方向        |
| **语言机制** | 借用了 C 的位移运算符，通过重载实现 |
| **优先级**   | 适中，多数情况符合预期              |
| **可扩展性** | 可为任意类型重载，类型安全          |
| **表达力**   | 支持链式调用，代码紧凑              |

所以，`<<` 和 `>>` 成为 C++ 输入输出的符号，是**视觉隐喻、运算符重载的巧妙复用、以及务实优先级选择**的共同遗产。虽然它们偶尔会因优先级问题让人困惑（例如忘记给位运算加括号），但整体上这个设计经受住了数十年实践的考验。 
