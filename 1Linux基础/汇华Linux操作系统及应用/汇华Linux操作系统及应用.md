...
- **`su` (Switch User )**：**切换用户**
- 核心作用是在不注销登录的情况下**临时切换**到另一个用户身份（默认为 `root`）。
- 验证时要求的是**目标用户的密码**。如果目标用户是 `root`，就必须输入 `root` 密码才能成功。这意味着，**任何知道 `root` 密码的用户，都能成功执行 `su`**，无论他是否在 `sudo` 组里。

## 转移当前工作用户

`su` 是 **S**witch **U**ser（切换用户）的缩写，核心作用是在不注销登录的情况下**临时切换**到另一个用户身份（默认为 `root`）。

```bash
#切换到 root，但保留当前目录和环境变量（半切换）
su
#切换到 root，并完整加载 root 的家目录和环境变量（推荐）。
su -
#切换到指定用户，并完整加载其环境。
su - username
	#转移到yishang用户的当前工作路径
...
执行 `su` 时，系统索要的是**目标用户的密码**。

- `su` → 需要输入 **root 密码**
- `su - yishang` → 需要输入 **yishang 的密码**

# 超级终端`sudo`

- **`sudo` (Superuser Do)**：验证时要求的是**执行者自己的密码**。成功与否，取决于该用户是否被授权（通常在 `sudo` 或 `wheel` 组）。**不在授权名单里，即使知道 `root` 密码，也无法使用 `sudo`。**
- 验证时要求的是**执行者自己的密码**。成功与否，取决于该用户是否被授权（通常在 `sudo` 或 `wheel` 组）。**不在授权名单里，即使知道 `root` 密码，也无法使用 `sudo`**
...
- **如果你知道 `root` 密码**：可以直接使用 `su -` 来切换到 `root` 用户。
- **如果你不知道 `root` 密码，但当前用户在 `sudo` 组里**：可以使用 `sudo -i` 或 `sudo su -` 来切换到 `root` 用户。执行时需要输入的是**你自己的密码**。
- **如果你既不知道 `root` 密码，也不在 `sudo` 组里**：那就需要联系系统管理员将你加入 `sudo` 组，或为你设置 `root` 密码了。

- **`-i`**：**模拟初始登录（simulate initial login）**。

| 命令          | 当前目录变化 | 环境变量                        | 适用场景                                             |
| :------------ | :----------- | :------------------------------ | :--------------------------------------------------- |
| **`sudo -i`** | 进入 `/root` | **纯净的 root 环境**            | 需要执行大量系统管理脚本，不想受原用户 PATH 干扰时   |
| `sudo su -`   | 进入 `/root` | **纯净的 root 环境**            | 效果几乎完全一样，但多启动了一个 `su` 进程，略显冗余 |
...
执行完上述命令后，**必须注销当前用户并重新登录**（或重启 SSH 会话），新权限才会生效。
...
如果在输出的列表里看到了 **`sudo`**，就说明用户已经拥有了 `sudo` 权限。
...
- 以下用户不属于sudo群组中,所以不能执行sudo命令

```bash
zhanghaoboo@zhanghaoboo-virtual-machine:/var/log$ id user1 
uid=1001(user1) gid=1001(user1) 组=1001(user1)
zhanghaoboo@zhanghaoboo-virtual-machine:sudo -i 
zhanghaoboo@zhanghaoboo-virtual-machine:user1 is not in the sudoers file.  This incident will be reported.
#用户 user1 没有被授予 sudo 权限,系统会将这次失败的提权尝试写入日志。
```
